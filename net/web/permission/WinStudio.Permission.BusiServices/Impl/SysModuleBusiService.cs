...
{
    [Export(typeof(ISysModuleBusiService))]
    public class SysModuleBusiService : AbstractPermissionBusiService<SysModule>, ISysModuleBusiService
    {
        public SysModuleBusiService() : base(new PermissionDbContext()) { }
        public SysModuleBusiService(PermissionDbContext context) : base(context) { }

        public ComRet GetAllUsingSysModules()
        {
            return Result(GetAll().Where(s => s.IsUsing).ToList());
        }
        ...
        public ComRet AutoGeneratePermission(string xmlpath, string syscode, string area)
        {
            ...
            {
                system.Roles.Add(new Role { Code = role.Code, Name = role.Name, Display = role.Display, ModuleID = system.ID });
            }
        ...
        private void ConvertToFunctions(List<Function> funcs, List<IPermissionNode> nodes, int sysid, string sysauth)
        {
            foreach (IPermissionNode node in nodes)
            {
                ...
                if (sysid > 0)
                    func.SysModuleID = sysid;
                func.IsUsing = true;
                ConvertToFunctions(func.Children, node.Children, 0, sysauth);
                funcs.Add(func);
            }
        }
        ...
        {
            var funcs = this.Get<PermissionDbContext>().Functions.Where(f => f.SysModuleID == sysid).ToList();
            var roles = this.Get<PermissionDbContext>().Roles.Where(f => f.ModuleID == sysid).ToList();
        ...
        public ComRet GetUserPermission(string sysauth, string syscode, string account)
        {
            //return null;
            RoleBusiService roleServ = new RoleBusiService();
            var rs = roleServ.GetAll(r => r.Module.Code == syscode && r.Module.Authority == sysauth && r.Customers.Any(c => c.Account == account)).ToList();

            if (rs == null || rs.Count == 0) return Result(string.Empty);
            ...
            return Result(true, sb.ToString());
        }

        public ComRet GetSysModule(int id)
        {
            return Result(GetById(id));
        }

        public ComRet SaveSysModule(SysModule module)
        {
            return Result(Save(module));
        }



        public ComRet AutoGeneratePermission(string xmlpath, int mid, string area)
        {
            var module = GetById(mid);
            if (module == null) return Result("不存在的系统！");

            DeleteSubSystemPermissionInDbContext(module.ID);
            ...
            foreach (IPermissionNode role in roles)
            {
                module.Roles.Add(new Role { Code = role.Code, Name = role.Name, Display = role.Display, ModuleID = module.ID });
            }
            ConvertToFunctions(module.Functions, tree.GetNodes(Permission.Principle.Mvc.PermissionNodeType.Function), module.ID, module.Authority);
        ...
        public ComRet Exists(int id)
        {
            return Result(Exist(id));
        }
    }
}
