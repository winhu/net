...
using WinStudio.Passport.Filters;
...
namespace WinStudio.Passport.Controllers
{
    ...
    {
        private ISignBusiService bsSign;
        public AccountController(ISignBusiService sign)
        {
            bsSign = sign;
        }
        ...
        public ActionResult Login(LoginModel model, string next)
        {
            ...
            {
                ComRet ret = bsSign.ValidateUser(model.UserName, model.Password);
                if (ret.Ret)
                {
                    next = next.HasValue() ? next.FromBase64String() : WebConsts.WinHandleProtalAddress;
                    Reception.FinishLogin(HttpContext, model.UserName, new Uri(next).GetAuthority());
        ...
        {
            return Reception.IsValidSession(HttpContext);
            //HttpContext.Response.Write(ret);
        }
        [HttpPost]
        //[ValidateAntiForgeryToken]
        public string UpdateInfo()
        {
            var xml = Reception.GetSession(HttpContext.GetToken()).ToXml();
        ...
        public ActionResult Register(RegisterModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    var ret = bsSign.RegisterUser(model.UserName, model.Password, model.Email);
                    if (!ret.Ret)
                        throw new Exception(ret.LastMsg);
